/**
 * Store - Signals, LocalForage, and Sync Logic
 * Workout plans are server-authoritative, logs use last-write-wins
 */
import { signal, batch } from '@preact/signals';
import localforage from 'localforage';
import { getToday, getUtcNow, generateId, deepClone } from './utils.js';
...
export async function triggerSync() {
    ...
    try {
        ...
        if (meta.dirtyDates.length > 0) {
            ...
            const uploadResponse = await fetch('/api/workout/sync', {
                method: 'POST',
        ...
        const downloadResponse = await fetch(`/api/workout/sync?${params}`);
